...
export async function reserve({ eventId, userId }: { eventId: string; userId: string }) {
  return prisma.$transaction(async (tx) => {
    ...
    const totalAfter = await tx.reservation.count({ where: { eventId, status: 'CONFIRMED' } });
    if (totalAfter >= event.maxCapacity) {
      getIO().emit("event_sold_out", { eventId, eventName: event.name });
    }
    getIO().emit("reservation_confirmed", { eventId, eventName: event.name, userId, reservationId: reservation.id });
...
export async function cancel({ reservationId, userId, isAdmin }: { reservationId: string; userId: string; isAdmin: boolean }) {
  ...
  if (!isAdmin && reservation.userId !== userId) throw new Error('Acesso negado');
